...
### Evolving the USELESS TRASH BIN into an autonomous companion without losing the plot
...
|---|---|---|---|
| **BIN-CHAD** | `firmware/BinChad/` + `firmware/BinRemote/` | ESP32-S3 (+ ESP32-C3 remote) | Complete modular firmware. **Never compiled** (root README says so). Stationary: no motors. |
| **TrashBotWeb** | `firmware/TrashBotWeb/` (+ `TrashBotCam/`) | Classic ESP32 (WROOM) | Builds clean under PlatformIO. **Never run on hardware.** Mobile: L298N + 2 DC motors, nothing else. |
...
|---|---|
| ESP32 control | BIN-CHAD (**S3**) and TrashBotWeb (**classic**). These are different chips with incompatible pin maps. |
| L298N + two geared DC motors | TrashBotWeb (ESP32 GPIO 18/19/27/26/25/33) **and** UNO Q sketches (D5–D10). The UNO Q wiring is newer. |
| Servo eyeball | BIN-CHAD `eye.*`: pan GPIO 5, tilt GPIO 6, eased, auto-detach |
| OLED face | BIN-CHAD `display.*` (I²C 8/9) and UselessBox_UnoQ (`Wire2`, custom SSD1306 driver) |
| ToF proximity | BIN-CHAD: 2× VL53L0X, **throat** (looks down into the bin) and **approach** (looks into the room) |
| Servo lid | BIN-CHAD `lid.*`: MG996R, soft-close, limit switches, obstruction reversal |
| Mechanical finger | BIN-CHAD `uselessFinger.*` (GPIO 7 + hatch GPIO 15); UselessBox (servo D11, switch D12) |
| WS2812B | BIN-CHAD `leds.*`, GPIO 16 |
| MAX98357A + speaker | BIN-CHAD `audio.*`: I²S 17/18/21, non-blocking WAV streaming from LittleFS |
| Personality engine | **Two different engines** (see §1.1) |
| Browser dashboard | TrashBotWeb only (`data/`: DRIVE, MIND, AUDIO, VISION, HW, SETUP tabs) |
| Browser-side computer vision | TrashBotWeb `data/vision.js`: COCO-SSD on TensorFlow.js, results sent to the ESP32 |
| ESP-NOW | BIN-CHAD ↔ BinRemote |
| Safety layer | BIN-CHAD: lid safety (hand detection, limit switches, servo power FET). TrashBotWeb: motor safety (latched E-stop, link/drive timeouts, ramp, duty ceiling). |
...
| Microphone | **Nowhere** |
| Battery measurement | **Nowhere** (TrashBotWeb reports `NOT INSTRUMENTED` on purpose) |
| Wheel encoders, IMU, cliff sensors, bumpers | **Nowhere** |
| Camera | Optional ESP32-CAM snapshot server (`TrashBotCam/`); phone camera |

### 1.1 The two personality engines

| | BIN-CHAD `personality.*` | TrashBotWeb `personality.*` |
|---|---|---|
...
The brief's trait list comes from TrashBotWeb, and its mood list from BIN-CHAD.
...
```
BIN-CHAD (ESP32-S3)                         TrashBotWeb (ESP32)
─────────────────────                       ────────────────────────────
...
|---|---|---|
| `hardware/eye.*` | BIN-CHAD | Body. `lookNormalized(x, y)` is exactly the `LOOK_AT` primitive. |
| `hardware/lid.*` | BIN-CHAD | Body. The safety logic is kept verbatim. |
| `hardware/uselessFinger.*` | BIN-CHAD | Body. The NORMAL MODE gag survives (§15.4). |
| `hardware/leds.*` | BIN-CHAD | Body. The pattern enum becomes the `LED_SET` vocabulary. |
| `hardware/audio.*` | BIN-CHAD | Body. Prerecorded clips are the offline voice (§14). |
| `ui/display.*` | BIN-CHAD | Body. `FaceExpression` + `say()` become `FACE_SET`. |
| `hardware/sensors.*` (`DebouncedInput`, ToF health) | BIN-CHAD | Body. Extended with new range sensors. |
| `motor/motors.*` | TrashBotWeb | Body. Ramp, duty ceiling, min duty, trim, NVS calibration. |
| `safety/safety.*` | TrashBotWeb | Body. Latch + timeouts. The link becomes the UNO Q instead of a browser. |
| `core/prng.h`, `core/eventLog.*` | TrashBotWeb | Both sides. The Python port of xorshift32 matches bit for bit (`brain/binbrain/prng.py`). |
| `data/` dashboard (HTML/CSS/JS, tab layout, style) | TrashBotWeb | Brain. Served from the UNO Q with new COMPANION and DEBUG tabs. |
...
| TrashBotWeb `pipeline.*` | Mistranslation applies **only to human-issued manual commands**, never to navigation output | A mistranslated obstacle-avoidance turn is a collision, not a joke |
| TrashBotWeb `safety.*` | "Client" = the UNO Q link; adds obstacle, cliff, bumper, battery and physical E-stop inhibits; autonomous vs manual speed ceilings | Autonomy needs sensor-based inhibits, not only link-based ones |
| BIN-CHAD `behaviorManager.*` | Split: hardware choreography stays on the body as named **gestures**; decisions move to the brain | The brief: "the ESP32 should not make high-level AI decisions" |
| BIN-CHAD `pins.h` | Needs motor, bumper, battery and E-stop pins (§6.2) | The S3 map has one free GPIO left |
| BIN-CHAD `remote.*` / BinRemote | Kept. On the mobile robot, remote **STOP always stops the motors** (it may still speed up the *eye*) | TrashBotWeb precedent: "It would not stop is not a joke, it is a fault report" |
| TrashBotWeb `vision.*` | Replaced on the body by the brain's world model; the phone/TF.js path stays as an optional input | The UNO Q camera is now the primary vision sensor |
...
|---|---|---|
| `binbrain.vision` | camera grabber, detector, appearance signature, tracker, geometry | 1 |
| `binbrain.world` | world model, person lifecycle, social zones, events, LLM summary | 1 |
| `binbrain.sim` | deterministic synthetic scenarios (tests + DEBUG "simulate a person") | 1 |
| `binbrain.body` | serial link, command validator, safety governor, telemetry | 2 |
| `binbrain.nav` | navigation state machine, reactive avoidance, person approach, docking | 3 |
| `binbrain.voice` | wake phrase, VAD, STT, TTS, audio routing, half-duplex policy | 4 |
| `binbrain.mind.llm` | `LLMProvider` → `CloudLLM`, `LocalLLM`, `OfflineFallback` | 5 |
| `binbrain.mind.conversation` | turn manager, output contract validation, memory | 5 |
| `binbrain.mind.personality` | unified deterministic engine | 5 |
| `binbrain.mind.social` | social decision system, cooldowns, target selection | 6 |
| `binbrain.mind.games`, `.stories` | deterministic game/story engines (no LLM required) | 6 |
| `binbrain.expression` | Emotion → PhysicalExpression mapping | 6–7 |
| `binbrain.dashboard` | HTTP + WebSocket server, COMPANION and DEBUG tabs | 2 → 7 |
...
|---|---|---|
| C1 | **Two ESP32 families.** BIN-CHAD uses GPIO 38/47/48, which do not exist on a classic ESP32. TrashBotWeb uses GPIO 25/26/27/33, which are SPI flash or PSRAM on an S3. | The two pin maps cannot be merged onto either chip as they stand. |
| C2 | **The L298N is documented in two places.** TrashBotWeb cites `tests/l298n_motor_test` as proof of its ESP32 wiring, but that test has since been rewritten for the UNO Q on D5–D10. | The repo does not say where the driver is physically wired today. |
| C3 | **The BIN-CHAD S3 map is full.** Of the always-safe set (1, 2, 4–18, 21, 38, 47, 48), only GPIO 1 is free. The companion needs 6 motor pins plus bumpers, battery sense and an E-stop input. | Motors cannot be added without freeing pins or adding an expander. |
...
| C9 | **BIN-CHAD remote STOP is deliberately mistranslated** (it speeds things up). | Must never apply to wheels. |
| C10 | **Motor noise vs. microphone; motor current vs. the UNO Q supply.** | Listening while driving is unreliable; brownouts can reboot the brain mid-drive (the TrashBotWeb README already warns about this for the ESP32). |
...
to I²C and frees GPIO 4/5/6/7/15. Every other BIN-CHAD pin stays where it is.

| GPIO | Today (BIN-CHAD) | Proposed |
|---:|---|---|
...
stays readable in a serial monitor, like TrashBotWeb's USB link:
...
| `hello` | `fw`, `v`, ceilings, `hw` inventory (same honesty rules as TrashBotWeb: `configured` vs `online` vs `not_installed`) |
...
| Dashboard | aiohttp/FastAPI + WebSocket, reusing TrashBotWeb's `data/` look | Existing UI philosophy | — |
...
- **Traits (0–100):** anger, trust, happiness, confusion, boredom, obedience,
  rebellion (derived, as in TrashBotWeb), **curiosity**, **social_energy**.
- **Moods:** BIN-CHAD's 13 states plus TrashBotWeb's BORED, REBELLIOUS, CHAOS
  and PANIC (17 total). Mood is derived from traits, except where a mode or
  event pins it.
- **Drive modes** (TrashBotWeb) remain a *manual driving* policy only.
- **Determinism:** the same xorshift32 stream, seed printed at boot and
  pinnable. `brain/binbrain/prng.py` is a bit-exact port of `core/prng.h`.
...
**Recommended: on the brain**, as a faithful Python port of TrashBotWeb's
...
| **2 · UNO Q ↔ ESP32** | `firmware/BinBody/` (drivers reused), protocol §9, reflex safety, `binbrain.body`, dashboard DRIVE + HW + DEBUG | **Wheels lifted.** Pull the USB cable while driving → stop < 300 ms; kill the brain process → stop; E-stop latch; corrupted CRC ignored; TTL expiry; the ceilings cannot be raised over the link |
...
1. **Which ESP32 is the body?** ESP32-S3 (the BIN-CHAD pin map, recommended for
   native USB and pins) or classic ESP32 (TrashBotWeb)? Which module (N8R2 /
   N16R8)?
