// =============================================================================
//  protocol.h - ESP-NOW link between BIN CONTROL SYSTEM v0.0001 and DUST-E.
//
...
#define DUSTE_PROTO_VERSION 1

// Device id, shared by both ends. A packet with any other id is ignored,
// so two DUST-Es can run in the same room by giving each pair its own id.
#define DUSTE_DEVICE_ID 0x2A
...
struct __attribute__((packed)) RemotePacket {
    uint8_t deviceId;   // must equal DUSTE_DEVICE_ID
    uint8_t version;    // DUSTE_PROTO_VERSION
    ...
    uint8_t checksum;   // see duste_checksum()
};
...
// ---------------------------------------------------------------------------
static inline uint8_t duste_checksum(const uint8_t *bytes, uint8_t len) {
    uint8_t x = 0;
    for (uint8_t i = 0; i < len - 1; ++i) x ^= bytes[i];
    return (uint8_t)(~x);
}

static inline void duste_sign(void *pkt, uint8_t len) {
    uint8_t *b = (uint8_t *)pkt;
    b[len - 1] = duste_checksum(b, len);
}

static inline bool duste_verify(const void *pkt, uint8_t len) {
    const uint8_t *b = (const uint8_t *)pkt;
    return b[len - 1] == duste_checksum(b, len);
}
...
// ---------------------------------------------------------------------------
static const uint8_t DUSTE_BROADCAST[6] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};

// Both devices must sit on the same channel when not associated with an AP.
#define DUSTE_ESPNOW_CHANNEL 1

static inline const char *duste_cmd_name(uint8_t c) {
    switch (c) {
