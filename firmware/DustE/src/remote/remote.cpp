...
static void handlePacket(const uint8_t *mac, const uint8_t *data, int len) {
    ...
    if (p.deviceId != DUSTE_DEVICE_ID)       { g_rejected++; return; }
    if (p.version  != DUSTE_PROTO_VERSION)   { g_rejected++; return; }
    if (p.type     != PKT_COMMAND)             { g_rejected++; return; }
    if (!duste_verify(&p, sizeof(p)))        { g_rejected++; return; }
...
void RemoteLink::sendAck(uint8_t ackSequence, uint8_t stateId, bool obeyed, uint8_t frustration) {
    BinStatusPacket s = {};
    s.deviceId    = DUSTE_DEVICE_ID;
    s.version     = DUSTE_PROTO_VERSION;
    s.type        = PKT_ACK;
    s.ackSequence = ackSequence;
    s.stateId     = stateId;
    s.obeyed      = obeyed ? 1 : 0;
    s.frustration = frustration;
    duste_sign(&s, sizeof(s));

    const uint8_t *dest = havePeer ? lastPeer : DUSTE_BROADCAST;
