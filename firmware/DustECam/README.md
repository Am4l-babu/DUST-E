...
The web remote in `../DustEWeb/` does its person-and-object detection in
...
By default it joins the bin's own access point (`DUST-E-SETUP` /
`dustebin`, from `DustEWeb/src/config/settings.h`), so the whole thing
