...
//  (DUSTE_ESPNOW_CHANNEL in protocol.h).
// =============================================================================
...
static void handleAck(const uint8_t *mac, const uint8_t *data, int len) {
    ...
    if (s.deviceId != DUSTE_DEVICE_ID) return;
    if (s.version  != DUSTE_PROTO_VERSION)  return;
    if (!duste_verify(&s, sizeof(s)))       return;
    ...
    if (!haveBin) {
        ...
        if (!esp_now_is_peer_exist(binMac)) {
            ...
            p.channel = DUSTE_ESPNOW_CHANNEL;
            p.encrypt = false;
            esp_now_add_peer(&p);
        }
    ...
    remoteUi.showAck(duste_cmd_name(pendingCmd), s.obeyed != 0, s.frustration);
...
static void transmit(uint8_t command, uint8_t seq, uint8_t repeat) {
    RemotePacket p = {};
    p.deviceId = DUSTE_DEVICE_ID;
    p.version  = DUSTE_PROTO_VERSION;
    p.type     = PKT_COMMAND;
    p.command  = command;
    p.sequence = seq;
    p.repeat   = repeat;
    p.flags    = 0;
    duste_sign(&p, sizeof(p));

    const uint8_t *dest = haveBin ? binMac : DUSTE_BROADCAST;
...
static void queueCommand(uint8_t command) {
    ...
    remoteUi.showSent(duste_cmd_name(command));
    setStatusLed(0, 0, 60);
    Serial.printf("[tx] %s seq=%u\r\n", duste_cmd_name(command), pendingSeq);
}
...
void setup() {
    ...
    esp_wifi_set_channel(DUSTE_ESPNOW_CHANNEL, WIFI_SECOND_CHAN_NONE);
    ...
    if (esp_now_init() != ESP_OK) {
        ...
        memcpy(bcast.peer_addr, DUSTE_BROADCAST, 6);
        bcast.channel = DUSTE_ESPNOW_CHANNEL;
        bcast.encrypt = false;
