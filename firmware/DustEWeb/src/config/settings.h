// =============================================================================
//  settings.h - every tunable number in the DUST-E web remote.
//
...
//   at http://dust-e.local (mDNS) or the raw IP printed on the serial port.
// ---------------------------------------------------------------------------
...
// included everywhere.
#define WIFI_HOSTNAME "dust-e"

// Where Wi-Fi credentials, motor calibration and the pinned seed live in
// flash. The value predates the DUST-E name and is kept on purpose: changing
// it makes an already-flashed board forget all of those on the next update.
#define NVS_NAMESPACE "trashbot"
...
#define AP_SSID_DEFAULT "DUST-E-SETUP"
#define AP_PASS_DEFAULT "dustebin"
...
// SOUND
//
//   Without HW_SPEAKER the phone that has the dashboard open is the speaker:
//   the firmware only ever sends a tiny "play this" message, and the browser
//   plays a clip it has already fetched. With HW_SPEAKER the bin plays the
//   clips it can decode itself - see ON-BOARD SPEAKER below. Clips are uploaded from the
...
// ---------------------------------------------------------------------------
// ON-BOARD SPEAKER (HW_SPEAKER)
//
//   The same clips, played on the bin itself through a MAX98357A. The board
//   decodes PCM WAV, MP3 (including MP3-inside-a-.wav, which is what most
//   phone recorders and converters actually produce) and ADTS AAC. Anything
//   else (.ogg, .opus, .webm, .m4a, .flac) is still sent to the phone.
//
//   SPK_PHONE_TOO: when the bin plays a clip itself, should the phone play it
//   as well? Off by default - the two are never in sync, and an echo sounds
//   like a fault. The dashboard still shows what the bin said.
// ---------------------------------------------------------------------------
static const uint8_t  SPK_VOLUME_PCT          = 70;      // 0..100, the amp has no volume pin
static const bool     SPK_PHONE_TOO           = false;
static const uint16_t SPK_TASK_STACK          = 12288;   // the AAC decoder is stack-hungry
static const uint8_t  SPK_QUEUE_LEN           = 4;

// ---------------------------------------------------------------------------
// EYES (HW_EYES)
//
//   Rendered in their own task at EYE_FRAME_MS, so a 50 ms I2C frame never
//   stalls the motor ramp or the safety timeouts in loop(). Each eye is
//   detected over I2C at boot; a missing one is reported, not faked.
// ---------------------------------------------------------------------------
static const uint8_t  EYE_ADDR_LEFT           = 0x3C;
static const uint8_t  EYE_ADDR_RIGHT          = 0x3D;    // 0x3C too if EYES_SPLIT_BUS
static const bool     EYES_SPLIT_BUS          = false;   // right eye on PIN_EYE_R_SDA/SCL
static const uint32_t EYE_I2C_HZ              = 400000;
static const uint16_t EYE_FRAME_MS            = 40;      // 25 fps target
static const uint16_t EYE_BLINK_MIN_MS        = 1800;
static const uint16_t EYE_BLINK_MAX_MS        = 5500;
static const uint16_t EYE_IDLE_LOOK_MS        = 2500;    // nobody steering the gaze -> glance around
static const uint16_t EYE_FLASH_MS            = 1400;    // default length of a reaction

// ---------------------------------------------------------------------------
// HANDS (HW_HANDS)
//
//   Angles are LOGICAL: 0 = arm hanging at its side, 180 = arm straight up,
//   for both arms. HAND_*_INVERT mirrors a servo mounted the other way round
//   so the gestures never have to know. Limits clamp every pose, so a bad
//   gesture table cannot drive an arm into the body.
//
//   Hands are not motors, but they are still moving parts next to fingers:
//   the emergency stop parks them slowly and then cuts their pulses, and the
//   personality engine cannot start a gesture while the stop is latched.
// ---------------------------------------------------------------------------
static const uint16_t HAND_PWM_HZ             = 50;
static const uint8_t  HAND_PWM_BITS           = 16;
static const uint16_t HAND_MIN_US             = 500;     // pulse at physical 0 deg
static const uint16_t HAND_MAX_US             = 2500;    // pulse at physical 180 deg
static const bool     HAND_LEFT_INVERT        = false;
static const bool     HAND_RIGHT_INVERT       = true;    // mirrored mounting
static const int8_t   HAND_LEFT_TRIM_DEG      = 0;
static const int8_t   HAND_RIGHT_TRIM_DEG     = 0;
static const uint8_t  HAND_LIMIT_LO_DEG       = 5;
static const uint8_t  HAND_LIMIT_HI_DEG       = 175;
static const uint16_t HAND_SLEW_DEG_PER_S     = 400;     // hard ceiling, whatever a gesture asks
static const uint16_t HAND_PARK_DEG_PER_S     = 60;      // how gently the e-stop parks them
static const uint16_t HAND_RELAX_MS           = 1500;    // at rest this long -> pulses off, no hum
static const uint16_t HAND_TICK_MS            = 20;
...
// ---------------------------------------------------------------------------
#define DUSTE_VISION_UART 1                            // listen on PIN_VISION_RX/TX
...
// ---------------------------------------------------------------------------
#define DUSTE_BLE       1
#define DUSTE_USB_LINK  1
#define BLE_DEVICE_NAME    "DUST-E"
...
static const bool HW_SPEAKER     = true;    // MAX98357A on the I2S pins. false -> the phone plays everything
static const bool HW_EYES        = true;    // two SSD1306 OLEDs. Probed at boot, so safe to leave on
static const bool HW_HANDS       = true;    // two servos. NOT detectable - set false if none are fitted
static const bool HW_DISPLAY     = false;   // a status display, separate from the eyes
