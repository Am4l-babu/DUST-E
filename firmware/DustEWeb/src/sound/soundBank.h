...
//
//  This module never touches audio data. The phone with the dashboard open is
//  the speaker, or the bin's own speaker is (speaker.h) for the clips it can
//  decode - either way this is only the bookkeeping: it holds the
...
struct SoundMsg {
    ...
    bool     onBoard;    // the bin's own speaker is playing it; phones just show it
};
...
class SoundBank {
  public:
    ...
    // Told about EVERY triggered event, clip or no clip, cooldown or not -
    // the eyes and hands react to events, not to sounds. loop() only.
    using Listener = void (*)(uint8_t event, uint32_t now);
