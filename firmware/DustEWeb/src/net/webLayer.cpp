...
<title>DUST-E (minimal)</title><style>
...
static void handleTextFrame(uint32_t clientId, const char* data, size_t len) {
    ...
    if (!strcmp(type, "ping")) {
        ...
        m.kind = MSG_GESTURE;
        m.a = parseGesture(doc["name"] | "");
        if (m.a >= GEST_COUNT) return;

    } else if (!strcmp(type, "face")) {
        m.kind = MSG_FACE;
        m.a = parseEyeExpr(doc["expr"] | "");
        if (m.a >= EXPR_COUNT) return;
...
static void sendHello(AsyncWebSocketClient* c) {
    ...
    // The speaker task can fail to start; the amp itself cannot be detected.
    hw["speaker"] = HW_SPEAKER     ? (speaker.ready() ? "configured" : "failed") : "see_telemetry";
    // The eyes answer on I2C, so they are the one fitted part that can
    // honestly be reported ONLINE or MISSING.
    hw["eyeL"]    = HW_EYES        ? (eyes.leftOk()  ? "online" : "missing") : "not_installed";
    hw["eyeR"]    = HW_EYES        ? (eyes.rightOk() ? "online" : "missing") : "not_installed";
    hw["hands"]   = HW_HANDS       ? "configured" : "not_installed";
    ...
    hw["usb"]     = DUSTE_USB_LINK ? "configured" : "not_installed";
    hw["visionUart"] = DUSTE_VISION_UART ? "configured" : "not_installed";
...
void WebLayer::sendSound(const SoundMsg& m) {
    ...
    doc["clip"]  = m.clip;
    // The bin is saying it itself; the phone shows it instead of echoing it.
    if (m.onBoard && !SPK_PHONE_TOO) doc["onBoard"] = true;
    sendDoc(doc, nullptr);
}
...
void WebLayer::broadcastTelemetry(uint32_t now) {
    ...
    body["speaking"]  = speaker.playing();
    body["gesture"]   = gestureName(hands.current());
    body["handsLive"] = hands.powered();
    body["handL"]     = hands.leftDeg();
    body["handR"]     = hands.rightDeg();
