...
This is a **separate machine** from DUST-E. DUST-E is an ESP32-S3 with
...
| `h2zero/NimBLE-Arduino` | 2.x (the Bluetooth trigger link; set `DUSTE_BLE 0` in `settings.h` to leave it out) |

### PlatformIO (what this was built and verified with)

```bash
cd firmware/DustEWeb
pio run              # build
...
Open `DustEWeb.ino`. Board: **ESP32 Dev Module**, Partition Scheme:
...
```
SSID      DUST-E-SETUP
password  dustebin          (change it in settings.h or the SETUP tab)
...
it in NVS (key `seed` in namespace `trashbot`, see `NVS_NAMESPACE`) to rehearse a demo or reproduce
...
| **ESP32-CAM snapshots** | Polls `http://trashcam.local/capture` a few times a second | The companion board in `firmware/DustECam/`, which joins the bin's AP and serves JPEGs with permissive CORS. No flag. |
...
| File | Contents |
|---|---|
| `DustEWeb.ino` | Wi-Fi, mDNS, and the loop that orders everything below. |
...
### The body kit: eyes, speaker, hands

Three parts already have drivers. Each one has its own flag, and all three
default to on:

| Flag | Part | Pins | Module |
|---|---|---|---|
| `HW_EYES` | 2 × SSD1306 128×64 I²C OLED (left 0x3C, right 0x3D) | SDA 21, SCL 22 (split bus: right eye on 5/15) | `src/face/` |
| `HW_SPEAKER` | MAX98357A I²S amp + 4–8 Ω speaker | BCLK 14, LRC 32, DIN 4 | `src/sound/speaker.*` |
| `HW_HANDS` | 2 × SG90/MG90S servo | left 13, right 23 | `src/hands/` |

`src/body/` maps every sound event to an eye expression and a gesture. The
mood sets the resting face. The emergency stop shows crossed-out eyes and
parks the arms, then turns their pulses off. The eyes are probed over I²C and
show up as `ONLINE`/`MISSING` on the HW tab. The speaker and servos cannot be
detected, so set their flags to `false` if they are not fitted. On the HW
tab, the BODY card has test buttons for every gesture and expression.

The speaker plays `.wav` (PCM, or MP3 inside a WAV), `.mp3` and ADTS `.aac`.
The phone still plays anything else. Servos and the amp need their own 5 V
rail with a common ground. Never power them from the ESP32's 5 V pin.

The expression gallery, wiring diagram and upgrade list are in
[docs/dust-e-body-kit.html](../../docs/dust-e-body-kit.html).
...
| `dust-e.local` does not resolve | Android does not do mDNS well. Use the IP from the serial port. |
