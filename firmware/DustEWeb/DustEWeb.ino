...
//
//  The phone is also the camera, and the speaker for anything the bin cannot
//  play itself. The bin never holds a camera frame: it sends a tiny "play
//  this" message (over Wi-Fi, BLE and USB serial, all at once) and receives
//  "I saw a person and a cup" reports from wherever the detector runs. See
//  src/sound/ and src/vision/.
//
//  Hardware for this build:  ESP32 dev module + L298N + 2 DC motors.
//  Optional body (src/body/): two SSD1306 OLED eyes, a MAX98357A speaker and
//  two servo hands - each switched in settings.h, none required.
//
...
//      -> SAFETY ENFORCE -> motor ramp -> body (eyes, hands) -> sounds for
//      what just changed
...
#include "src/sound/speaker.h"
#include "src/body/body.h"
#include "src/face/eyes.h"
#include "src/hands/hands.h"
...
void setup() {
    ...
    eventLog.begin();
    eventLog.push("BOOT", "DUST-E OS v%s starting", DUSTE_VERSION);
    ...
    soundBank.begin();
    soundBank.setListener([](uint8_t ev, uint32_t t) { body.react(ev, t); });
    speaker.begin();
    body.begin();
    vision.begin();
...
void loop() {
    ...
    while (webLayer.popInbound(m)) {
        switch (m.kind) {
            ...
            case MSG_SOUND_TEST:
                ...
                break;
            case MSG_GESTURE:
                if (body.testGesture(m.a, now)) eventLog.push("BODY_TEST", "gesture %s", gestureName(m.a));
                else eventLog.push("BODY_TEST", "gesture %s refused (no hands, or e-stop)", gestureName(m.a));
                break;
            case MSG_FACE:
                if (body.testFace(m.a)) eventLog.push("BODY_TEST", "face %s", eyeExprName(m.a));
                else eventLog.push("BODY_TEST", "face %s: no eyes online", eyeExprName(m.a));
                break;
            case MSG_PING:
    ...
    // One message, three links. The phone de-duplicates on seq. If the bin's
    // own speaker takes the clip, the browser is only told about it, and the
    // BLE/USB phones - which cannot be told "just show it" - are skipped.
    SoundMsg s;
    while (soundBank.pop(s)) {
        s.onBoard = speaker.play(s.clip, s.event == SND_ESTOP);
        webLayer.sendSound(s);
        if (s.onBoard && !SPK_PHONE_TOO) continue;
        bleLink.sendSound(s);
        usbLink.sendSound(s);
    }
