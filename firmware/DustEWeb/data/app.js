...
var HW_LABELS = {
  ...
  eyeL: 'LEFT EYE (OLED)', eyeR: 'RIGHT EYE (OLED)', hands: 'SERVO HANDS'
};

// Must match kNames in src/hands/hands.cpp and src/face/eyes.cpp.
var GESTURES = ['wave', 'shrug', 'celebrate', 'angry', 'flail', 'refuse', 'poke', 'bow',
                'facepalm', 'point', 'sleep', 'rest'];
var FACES = ['neutral', 'happy', 'bored', 'confused', 'angry', 'rebellious', 'sleeping',
             'chaos', 'panic', 'surprised', 'suspicious', 'sad', 'love', 'estop'];

function buildBodyButtons() {
  function fill(id, list, type, key) {
    $(id).innerHTML = list.map(function (n) {
      return '<button class="tbtn" data-' + type + '="' + n + '">' + n + '</button>';
    }).join('');
    $(id).addEventListener('click', function (ev) {
      var b = ev.target.closest('button');
      if (!b) return;
      var msg = { type: type };
      msg[key] = b.getAttribute('data-' + type);
      send(msg);
    });
  }
  fill('gestureBtns', GESTURES, 'gesture', 'name');
  fill('faceBtns', FACES, 'face', 'expr');
}
...
function renderBody(m) {
  ...
  if (!b) return;
  var rows = [
    ['SPEAKER', b.speaking ? 'TALKING' : 'quiet'],
    ['GESTURE', b.gesture],
    ['HANDS', b.handsLive ? ('L ' + b.handL + '° / R ' + b.handR + '°') : 'relaxed (no pulses)']
  ];
  $('bodyInfo').innerHTML = rows.map(function (r) {
    return '<div><span>' + r[0] + '</span><b>' + esc(String(r[1])) + '</b></div>';
  }).join('');
}
